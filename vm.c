...
swap(struct proc* p){
  ...
  int i;
  pte_t* pte = walkpgdir(p->pgdir,(char*)PGROUNDDOWN(p->plist.frames[j].va),0);
  for(i=0; i<MAX_PSYC_PAGES; i++){
    if(p->plist.used_file[i] == 0){
      p->plist.frames[j].swapFile_offset = i;
      p->plist.used_file[i] = 1;
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    if(phys_counter ==  MAX_PSYC_PAGES && proc->pid >2){
     if(file_counter ==  MAX_PSYC_PAGES){
        cprintf("no more space in the file\n");
    ...
    if(proc->pid >2){
      ...
      if(add_page(proc,pte,PGROUNDDOWN(a),-1) == 0){//adding page to our list when pte is his 
        cprintf("memory overflow 30 pages\n");
        deallocuvm(pgdir, newsz, oldsz);
        return 0;
      }
    }
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((mem = kalloc()) == 0)//the page is present, allocating physcal memory in child
      goto bad;
...
void
pgflt_handler(struct proc* p, uint va_fault){
  p->plist.pgflt_counter++;
  pte_t* pte = walkpgdir(p->pgdir, (char*)PGROUNDDOWN(va_fault), 0);
  if(!(*pte & PTE_PG)){//the page wasn't paged out
    panic("the page wasn't paged out\n");
  }
  else{
    int i;
    for(i = 0; i<MAX_TOTAL_PAGES; i++){
      if(p->plist.frames[i].used == 0){
        continue;
      }
      if(PGROUNDDOWN(p->plist.frames[i].va) == PGROUNDDOWN(va_fault)){
        if(p->plist.frames[i].swapFile_offset == -1){
          panic("problem - the page does not have swapFile_offset\n");
        }
        
      int phys_counter = counts_phys_memory(proc);
      if(phys_counter == 15 && proc->pid >2){
        swap(p);
      }

      char* mem = kalloc();
      readFromSwapFile(p, mem , p->plist.frames[i].swapFile_offset*PGSIZE, PGSIZE);
      mappages(p->pgdir, (char*)PGROUNDDOWN(p->plist.frames[i].va), PGSIZE, v2p(mem), PTE_W|PTE_U|PTE_P);
      pte = walkpgdir(p->pgdir, (char*)p->plist.frames[i].va,0);
      p->plist.used_file[p->plist.frames[i].swapFile_offset] = 0;
      p->plist.frames[i].swapFile_offset = -1;
      *pte = *pte & ~PTE_PG;
      break;
      }
    }
    if(i == MAX_TOTAL_PAGES){
      panic("virtual address of page fault does not exist\n");
    }
  }
